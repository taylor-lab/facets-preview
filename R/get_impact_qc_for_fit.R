...
get_impact_qc_for_fit = function(facets_output) {
  ...
  st = facetsSuite::check_fit(facets_output, genome = 'hg19', algorithm = 'em')
  
  ###
  ### Filter 1: clonal homdels should be < 2% of the autosomal genome
  ###
  homdel_filter_pass = ifelse(st$frac_homdels_clonal < 0.02, T, F)
  homdel_filter_note = paste0('% genome clonal-homdel: ', round(st$frac_homdels_clonal * 100, 2), 
                              '% (expected <2%), and, % genome (any)-homdel: ', round(st$frac_homdels * 100, 2), 
                              '% (expected <5%)') 
    
  ###
  ### Filter 2: 'number of' and fraction of genome within balanced and imbalanced diploid regions.
  ###
  diploid_bal_seg_filter_pass = ifelse(st$frac_dip_bal_segs > 0.01 & st$n_dip_bal_segs > 0, T, F)
  diploid_imbal_seg_filter_pass = ifelse(st$frac_dip_imbal_segs > 0.05 & st$n_dip_imbal_segs > 1, T, F)
  diploid_seg_filter_note = 
    paste0('One of these should be true:',
           '\tfrac. of diploid genome that is balanced: ', round(st$frac_dip_bal_segs * 100, 2), '% (expected: atleast 1%)\n',
           '\t# of segments that are diploid and balanced: ', st$n_dip_bal_segs, ' (expected: at least 1)\n',
           '\nor\n',
           '\tfrac. of diploid genome that is imbalanced: ', round(st$frac_dip_imbal_segs * 100, 2), '% (expected: atleast 5%)\n',
           '\t# of segments that are diploid and imbalanced: ', st$n_dip_imbal_segs, ' (expected: at least 2)\n')

  ###
  ### Filter 3: Waterfall Flag: pattern where the variance of logR ratio is very high; 
  ### typically attributed to assay artifact
  ###
  waterfall_filter_pass = ifelse((is.na(facets_output$purity) | facets_output$purity < 0.5) && st$sd_cnlr_residual > 1, F, T)
  waterfall_filter_note = paste0('SD of residuals from cnlr: ', round(st$sd_cnlr_residual, 3), ' (expected atleast 50% purity or sd_cnlr_residual < 1)')
  ...
  hyper_seg_filter_pass = ifelse(st$n_segs > 65 &&
                                   (st$n_dip_bal_segs + st$n_dip_imbal_segs) < 5 && 
                                   max(st$frac_dip_bal_segs, st$frac_dip_imbal_segs) < 0.1, F, T)
  hyper_seg_filter_note = paste0('# segments: ', st$n_segs, ' (fail if n_segs > 65 and insufficient fraction of the genome that is diploid)')
  ...
  ###
  high_ploidy_filter_pass = ifelse( facets_output$ploidy > 5 && (facets_output$ploidy > 7 || facets_output$purity < 0.1 || st$frac_dip_bal_segs < 0.05), F, T)
