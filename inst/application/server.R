...
function(input, output, session) {
  ...
  observeEvent(input$button_saveChanges, {
    ...
    df <-
      fread(cncf_filename) %>%
      select(-(tcn:lcn.em)) %>%
      left_join(df %>% data.table %>% select(chrom, loc.start, loc.end, seg, tcn:lcn.em),
  ...
  #      close.up(out, fit, gene.name=selected_gene, cached.gene.path = '/ifs/work/bandlamc/git/facets-suite/Homo_sapiens.GRCh37.75.gene_positions.txt')
